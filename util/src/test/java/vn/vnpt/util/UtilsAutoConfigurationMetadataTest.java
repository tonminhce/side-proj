...
/**
 * Guards the Spring Boot 4 autoconfig contract that downstream services depend on.
 * Story 0.1 / R-01 removed the inherited {@code <parent>} pom; this test fails loudly
 * if that cleanup accidentally deletes the autoconfig metadata or the entry-point class.
 */
class UtilsAutoConfigurationMetadataTest {
	...
	void autoconfigImportsFileRegistersUtilsAutoConfiguration() throws Exception {
		try (InputStream in = getClass().getClassLoader().getResourceAsStream(AUTOCONFIG_IMPORTS)) {
			assertNotNull(in, AUTOCONFIG_IMPORTS + " must be on the classpath (Spring Boot 4 autoconfig contract)");
			String contents;
	...
	void utilsAutoConfigurationClassIsAnnotatedAsSpringConfiguration() {
		assertNotNull(UtilsAutoConfiguration.class.getAnnotation(Configuration.class),
				UtilsAutoConfiguration.class.getName() + " must carry @Configuration for Spring to pick it up");
	}
}