...
/**
 * Tiện ích chuẩn hóa thông điệp lỗi SQL, ưu tiên ánh xạ theo SQLState PostgreSQL
 * và bổ sung metadata bảng/trường nếu có trong thông điệp gốc.
 */
@Slf4j
public final class SqlExceptionMessageResolver {
    ...
    /**
     * Chuẩn hóa thông điệp lỗi SQL theo chiến lược ưu tiên:
     * 1) Ưu tiên ánh xạ SQLState PostgreSQL.
     * 2) Fallback phân tích keyword trong thông điệp gốc.
     * 3) Không xác định được thì trả về thông điệp mặc định.
     *
    ...
    public static String resolvePostgresSqlStateMessage(Throwable throwable) {
        ...
        return switch (sqlState) {
            case "23505" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_UNIQUE_CONSTRAINT);
            case "23503" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_FOREIGN_KEY_CONSTRAINT);
            case "23502" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_NOT_NULL_CONSTRAINT);
            case "23514" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_CHECK_CONSTRAINT);
            case "22001" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_LENGTH_EXCEEDED);
            case "22P02" -> buildSqlConstraintMessage(throwable,
                    SqlExceptionConstant.EXCEPTION_SQL_INVALID_TYPE_FORMAT);
            default -> null;
        };
    }
    ...
    private static String resolveByKeyword(Throwable throwable, String defaultMessage) {
        ...
        if (causeMsg.contains("duplicate") || causeMsg.contains("unique")) {
            return buildSqlConstraintMessage(throwable, SqlExceptionConstant.EXCEPTION_SQL_UNIQUE_CONSTRAINT);
        }
        if (causeMsg.contains("foreign key") || causeMsg.contains("violates foreign key")) {
            return buildSqlConstraintMessage(throwable, SqlExceptionConstant.EXCEPTION_SQL_FOREIGN_KEY_CONSTRAINT);
        }
        if (causeMsg.contains("not-null") || causeMsg.contains("null value in column")) {
            return buildSqlConstraintMessage(throwable, SqlExceptionConstant.EXCEPTION_SQL_NOT_NULL_CONSTRAINT);
        }
        if (causeMsg.contains("check constraint") || causeMsg.contains("violates check")) {
            return buildSqlConstraintMessage(throwable, SqlExceptionConstant.EXCEPTION_SQL_CHECK_CONSTRAINT);
        }
    ...
    public static String buildSqlConstraintMessage(Throwable throwable, String fallbackMessage) {
        ...
        if (sqlException != null) {
            log.debug("SQLState: {}, ErrorCode: {}", sqlException.getSQLState(), sqlException.getErrorCode());
        }

        if (tableName != null && fieldName != null) {
            return String.format(SqlExceptionConstant.EXCEPTION_SQL_METADATA_TABLE_FIELD_TEMPLATE,
                    fallbackMessage,
                    tableName,
                    fieldName);
        }
        if (tableName != null) {
            return String.format(SqlExceptionConstant.EXCEPTION_SQL_METADATA_TABLE_TEMPLATE,
                    fallbackMessage,
                    tableName);
        }
        if (fieldName != null) {
            return String.format(SqlExceptionConstant.EXCEPTION_SQL_METADATA_FIELD_TEMPLATE,
                    fallbackMessage,
                    fieldName);
        }
