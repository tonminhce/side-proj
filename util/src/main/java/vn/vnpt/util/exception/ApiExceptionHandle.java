...
public class ApiExceptionHandle {
    ...
    public ExceptionMessage checkValidException(MethodArgumentNotValidException ex) {
        ...
        ex.getBindingResult().getFieldErrors().forEach(err ->
                invalidInputs.put(err.getField(), err.getDefaultMessage())
        );

        String message = invalidInputs.values().stream().collect(Collectors.joining(". "));
        return new ExceptionMessage(HttpStatus.BAD_REQUEST.value(), StringUtils.capitalize(message), invalidInputs);
    }
    ...
    */
    ...
    public ExceptionMessage checkValidDataException(ResponseStatusException ex) {
        return new ExceptionMessage(
                ...
                StringUtils.capitalize(ex.getReason()));
    }
    ...
    public ExceptionMessage checkValidParamException(BindException ex) {
        ...
        String message = errors.stream().map(DefaultMessageSourceResolvable::getDefaultMessage)
    ...
    public ExceptionMessage MaxUploadSizeExceededException(Exception ex, WebRequest request) {
        log.error(ex.getMessage());
        return new ExceptionMessage(HttpStatus.PAYLOAD_TOO_LARGE.value(), "File upload quá dung lượng 50MB cho phép.");
    }
    ...
    public ExceptionMessage handleAccessDeniedException(AccessDeniedException ex, WebRequest request) {
    ...
    /**
     * Xử lý lỗi vi phạm ràng buộc toàn vẹn dữ liệu SQL (unique constraint, foreign key, not null, v.v.)
     *
    ...
    public ExceptionMessage handleDataIntegrityViolationException(DataIntegrityViolationException ex) {
        ...
        String message = SqlExceptionMessageResolver.resolveSqlConstraintMessage(
                cause,
                SqlExceptionConstant.EXCEPTION_SQL_CONSTRAINT_GENERIC
        );
    ...
    public ExceptionMessage handleDuplicateKeyException(DuplicateKeyException ex) {
        ...
        String message = SqlExceptionMessageResolver.resolveSqlConstraintMessage(
                cause,
                SqlExceptionConstant.EXCEPTION_SQL_DUPLICATE_GENERIC
        );
        return new ExceptionMessage(HttpStatus.CONFLICT.value(), message);
    }
    ...
    public ExceptionMessage handleBadSqlGrammarException(BadSqlGrammarException ex) {
        ...
        return new ExceptionMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), SqlExceptionConstant.EXCEPTION_SQL_BAD_GRAMMAR);
    }
    ...
    public ExceptionMessage handleDeadlockException(DeadlockLoserDataAccessException ex) {
        ...
        return new ExceptionMessage(HttpStatus.CONFLICT.value(), SqlExceptionConstant.EXCEPTION_SQL_DEADLOCK);
    }
    ...
    public ExceptionMessage handleCannotAcquireLockException(CannotAcquireLockException ex) {
        ...
        return new ExceptionMessage(HttpStatus.CONFLICT.value(), SqlExceptionConstant.EXCEPTION_SQL_CANNOT_ACQUIRE_LOCK);
    }
    ...
    public ExceptionMessage handleDataAccessException(DataAccessException ex) {
        ...
        return new ExceptionMessage(HttpStatus.SERVICE_UNAVAILABLE.value(), SqlExceptionConstant.EXCEPTION_SQL_DATA_ACCESS_UNAVAILABLE);
    }
    ...
    public ExceptionMessage handleTypeMismatchException(Exception ex) {
        ...
        if (ex instanceof MethodArgumentTypeMismatchException mismatchEx) {
            message = String.format("Tham số '%s' có giá trị '%s' không đúng định dạng yêu cầu", 
