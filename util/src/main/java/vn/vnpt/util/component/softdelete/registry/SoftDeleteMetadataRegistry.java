...
/**
 * Nguồn dữ liệu duy nhất cho metadata {@code @SoftUk}, build 1 lần lúc app start bằng cách quét
 * {@link EntityManagerFactory#getMetamodel()} — chỉ những entity Hibernate thực sự đăng ký mới được
 * index, không cần classpath scanning riêng.
 *
 * <p>Đồng thời cache field {@code @Id} của mỗi entity (PK trong project này là {@code uuid} kiểu
 * {@code Long} khai báo ở {@code BaseEntity}, nhưng registry tra theo annotation {@code @Id} thay
 * vì hardcode tên field, để không phụ thuộc convention đặt tên).
 */
@Slf4j
@Component
...
public class SoftDeleteMetadataRegistry {
  ...
  void build() {
        ...
        for (int i = 0; i < uk.fields().length; i++) {
          ...
          if (f == null) {
            throw new IllegalStateException(
                "@SoftUk trên "
                    + entity.getName()
                    + " tham chiếu field không tồn tại '"
                    + fName
                    + "'");
          }
    ...
    log.info(
        "[SoftDelete] UK registry built: {} nhóm trên {} entity",
        totalUks,
        this.uksByEntity.size());
  }
  ...
  /**
   * @return field mang {@code @Id} của entity (đã setAccessible), hoặc {@code null} nếu entity
   *     không có trong metamodel / không tìm thấy field
   */
