...
public class PathConverter {
    ...
    private static Map<String, String> getDictionary() {
        try {
            ...
            try (InputStream in = classLoader.getResourceAsStream(CSV_JSON_FILE); BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(in)))) {
                ...
                throw new Exception(Paths.get(Objects.requireNonNull(classLoader.getResource(CSV_JSON_FILE)).toURI()).toString());
            }
    ...
    public boolean hasAuthority(String authority, Collection<String> source) {
        return AuthorityUtils.hasAuthority(authority, source) || AuthorityUtils.hasAuthority(authority, convertCodeToStringPermission(source));
    }
