        ...
        try {
            ...
            if (reqBody == null ||  reqBody.isEmpty()) {
                if (errorMessageObj.getWrappedReq().getInputStream().readAllBytes().length > 0) {
                    reqBody = new String(errorMessageObj.getWrappedReq().getInputStream().readAllBytes(), StandardCharsets.UTF_8);
                }
            }
            ...
            stringBuilder.append(code)
                    ...
                    .append("</strong>");
            File tempRequestBodyFile = CommonUtil.getTemplateFile(CommonConstant.TELEGRAM_TEMPORARY_FOLDER + code + "_REQUEST_BODY_LOG.txt");
            Files.writeString(tempRequestBodyFile.toPath(), Objects.requireNonNull(reqBody));
            File tempResponseBodyFile = CommonUtil.getTemplateFile(CommonConstant.TELEGRAM_TEMPORARY_FOLDER + code + "_RESPONSE_BODY_LOG.txt");
