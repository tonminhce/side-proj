...
public class CDPApiUtil {
    public CDPTokenResultDto getTokenByAccountAndPassword(String url, CDPUserDto user) {
        ...
        try {
            ...
            headers.setBearerAuth(Objects.requireNonNull(jwtAuthenticationToken).getToken().getTokenValue());
            ...
            if (result.getStatusCode() != HttpStatus.OK) {
                return new CDPTokenResultDto(ErrorCode.UNKNOWN_ERROR.getValue(), null, "Lấy thông tin token thất bại");
            }
            return new CDPTokenResultDto(ErrorCode.SUCCESS.getValue(), Objects.requireNonNull(result.getBody()).get("access_token").toString(), "Lấy thông tin token thành công");
        } catch (Exception e) {
            return new CDPTokenResultDto(ErrorCode.UNKNOWN_ERROR.getValue(), null, "Lấy thông tin token thất bại");
        }
    }
