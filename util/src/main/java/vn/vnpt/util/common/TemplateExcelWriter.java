...
public class TemplateExcelWriter {
    ...
    public <T> File setSingleDataToTemplate(T data, String dynamicSheetName, String dynamicTitle, List<Integer> removeColumns) {
        return this.setSingleDataToTemplate(data, dynamicSheetName, dynamicTitle, removeColumns, null);
    }

    public <T> File setSingleDataToTemplate(T data, String dynamicSheetName, String dynamicTitle, List<Integer> columnsToRemove, List<Integer> columnsToHide) {
        return this.setSingleDataToTemplate(data, dynamicSheetName, dynamicTitle, columnsToRemove, columnsToHide, 1, 1);
    }

    public <T> File setSingleDataToTemplate(T data, String dynamicSheetName, String dynamicTitle, List<Integer> columnsToRemove, List<Integer> columnsToHide, Integer dynamicTitleRowIndex, Integer dynamicTitleCellIndex) {
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(ExcelExportForm.class)) {
                    ...
                    if (CollectionUtils.isNotEmpty(list)) {
                        this.setDataInListToTemplate(list, startRow, sheet, config.hasSampleRow(), list.get(0).getClass());
                    }
                }
            }
    ...
    /**
     * Xóa cột tại vị trí columnIndex trong sheet.
     * Các cột bên phải sẽ được dịch chuyển sang trái để lấp đầy khoảng trống.
     * Xử lý các ô gộp (merged cells) trước khi dịch chuyển.
     */
    public void removeColumn(Sheet sheet, int columnIndex) {
        ...
        if (columnIndex < 0 || columnIndex > lastColumn) {
            log.warn("Column index {} is out of bounds (max: {}). Skipping removal.", columnIndex, lastColumn);
            return;
        }
    ...
    public File getTemplateFile(String fileName) {
        ...
        try {
            ...
            log.error("Lỗi sao chép tệp: " + e.getMessage() + ", đường dẫn: " + (tempFile != null ? tempFile.getAbsolutePath() : "null"));
    ...
    protected <T> void setSingleDataToSheet(T data, Sheet sheet, boolean hasSamepleRow, boolean clearExistingListData) {
        if (data == null) {
            return;
        }
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(ExcelExportForm.class)) {
                    ...
                    if (List.class.isAssignableFrom(field.getType())) {
                        if (Objects.requireNonNull(fa).compareSheetName() && !fa.sheetName().equals(sheet.getSheetName())) {
                            continue;
                        }
                        int startRow = Objects.requireNonNull(fa).startRow();
                        List<?> list = (List<?>) FieldUtils.readField(field, data, true);
                        if (CollectionUtils.isNotEmpty(list)) {
                            this.setDataInListToTemplate(list, startRow, sheet, hasSamepleRow, list.get(0).getClass(), clearExistingListData);
                        } else if (clearExistingListData) {
                            Class<?> elementType = getListElementType(field);
                            if (elementType != null) {
                                this.setDataInListToTemplate(list, startRow, sheet, hasSamepleRow, elementType, true);
                            }
                        }
                    } else {
    ...
    protected void setValueToCell(Object value, Cell cell, ValueDefinition def) {
        ...
        if (value instanceof Long || value instanceof Double || value instanceof BigDecimal || value instanceof Integer || value instanceof BigInteger) {
    ...
    protected void setDataInListToTemplate(List<?> listDataInSheet, int startRow, Sheet sheet
            , boolean hasSampleRow, Class<?> type, boolean clearExistingData) {
    ...
    public void setDataToCell(Object value, Cell cell) {
        ...
        if (value instanceof Long || value instanceof Double || value instanceof BigDecimal || value instanceof Integer || value instanceof BigInteger || value instanceof Float) {
            BigDecimal tmp = new BigDecimal(value.toString());
            cell.setCellValue(tmp.doubleValue());
        } else if (value instanceof String) {
            cell.setCellValue((String) value);
    ...
    public void setStyleForCell(Sheet sheetData, XSSFCellStyle cellStyle, Integer colIdx, Integer rowIdx) {
    ...
    public void setDataToCategorySheet(XSSFSheet sheet, List<Map<String, Object>> categories, List<String> keyList) {
        int dataRowStart = 0;
    ...
    /**
     * @param params: the object param that contain keys "keyList", "pass", "firstRow", "lastRow", "firstCol", "lastCol"
     */
    public void createCategorySheet(XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet,
    ...
    public void createCategorySheetWithReference(XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet,
                                                 List<Map<String, Object>> categories,
                                                 List<Map<String, Object>> refCategories, Map<String, Object> params) {
        @SuppressWarnings("unchecked")
        List<String> keyList = (List<String>) params.get("keyList");
        String pass = (String) params.get("pass");
        ...
        if (isGenerateData) {
            ...
            for (Map<String, Object> ref : refCategories) {
                List<Map<String, Object>> _categories = categories.stream()
                ...
                nameTmp.setNameName(srcSheetName + '_' + (ref.get("id") != null ? ref.get("id") : ref.get("code")));
        ...
        String refFormulas = "";
        if (isExtendRef) {
            refFormulas += "INDIRECT(\"" + importSheetName + "!\"&ADDRESS(ROW()-" + firstRow + ",COLUMN()-" + distanceRef + "))";
        } else {
            refFormulas += "INDEX(" + refSheetName + "!$A$1:$A$" + refCategories.size() +
                    ",MATCH(INDIRECT(ADDRESS(ROW(),COLUMN()-" + distanceToRef + "))," + refSheetName + "!$B$1:$B$" + refCategories.size() + ",0))";
        }
...
// dẫn đến danh sách thôn/
    public void createCategorySheetWithReferenceCode(XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet,
                                                     List<Map<String, Object>> categories,
                                                     List<Map<String, Object>> refCategories, Map<String, Object> params) {
        @SuppressWarnings("unchecked")
        List<String> keyList = (List<String>) params.get("keyList");
        ...
        String refFormulas = "INDEX(" + refSheetName + "!$A$1:$A$" + refCategories.size()
                + ",MATCH(INDIRECT(ADDRESS(ROW(),COLUMN()+" + 1 + "))," + refSheetName + "!$A$1:$A$" + refCategories.size() + ",0))";
        String formulas = "INDIRECT(\"" + srcSheetName + "_\"&" + refFormulas + ")";
        setXSSFValidation(desSheet, formulas, params);
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(ExcelExportForm.class)) {
                    ...
                    if (List.class.isAssignableFrom(field.getType())) {
                        ...
                        List<?> list = (List<?>) FieldUtils.readField(field, data, true);
                        if (CollectionUtils.isNotEmpty(list)) {
                            this.setDataInListToTemplateMergeRow(list, startRow, sheet, hasSamepleRow, list.get(0).getClass());
                        }
                    } else {
    ...
    protected void setDataInListToTemplateMergeRow(List<?> listDataInSheet, int startRow, Sheet sheet
            , boolean hasSampleRow, Class<?> type) {
        if (CollectionUtils.isEmpty(listDataInSheet)) {
            return;
        }
                    ...
                    if (columnIndexArray.length > 0) {
                        for (int index : columnIndexArray) {
                            if (index > 0) {
                                if (fa.isNotMergeCell()) {
                                    ...
                                    if (isMergeRow) {
                                        sheet.addMergedRegion(new CellRangeAddress(excelData.getStartMergeIndex(), excelData.getEndMergeIndex(), index, index));
                                    } else {
                                        this.getCellAndWriteValue(value, excelData.getRow(), index, def);
                                    }
                                }
                        ...
                        if (columnIndex > 0) {
                            if (fa.isNotMergeCell()) {
                                ...
                                if (isMergeRow) {
                                    sheet.addMergedRegion(new CellRangeAddress(excelData.getStartMergeIndex(), excelData.getEndMergeIndex(), columnIndex, columnIndex));
                                } else {
                                    this.getCellAndWriteValue(value, excelData.getRow(), columnIndex, def);
                                }
                            }
    ...
    public void setXSSFListValidation(XSSFSheet sheet, String formulaString, int firstRow, int lastRow, int firstCol, int lastCol) {
        DataValidationHelper helper = sheet.getDataValidationHelper();
        CellRangeAddressList regions = new CellRangeAddressList(firstRow, lastRow, firstCol, lastCol);
    ...
    public void setXSSFIntegerValidation(XSSFSheet sheet, int firstRow, int lastRow, int firstCol, int lastCol, Long fromValue, Long endValue) {
        DataValidationHelper helper = sheet.getDataValidationHelper();
        CellRangeAddressList regions = new CellRangeAddressList(firstRow, lastRow, firstCol, lastCol);
        DataValidationConstraint constraint = helper.createIntegerConstraint(DataValidationConstraint.OperatorType.BETWEEN, fromValue.toString(), endValue.toString());
        DataValidation dataValidation = helper.createValidation(constraint, regions);

        if (dataValidation instanceof XSSFDataValidation) {
            if (fromValue >= 0) {
                dataValidation.createErrorBox("Thông báo lỗi", "Định dạng dữ liệu phải là số nguyên không âm!");
                dataValidation.setShowErrorBox(true);
            } else {
    ...
    public void setXSSFDecimalValidation(XSSFSheet sheet, int firstRow, int lastRow, int firstCol, int lastCol, Double fromValue, Double endValue) {
        DataValidationHelper helper = sheet.getDataValidationHelper();
        CellRangeAddressList regions = new CellRangeAddressList(firstRow, lastRow, firstCol, lastCol);
        DataValidationConstraint constraint = helper.createDecimalConstraint(DataValidationConstraint.OperatorType.BETWEEN, fromValue.toString(), endValue.toString());
        DataValidation dataValidation = helper.createValidation(constraint, regions);

        if (dataValidation instanceof XSSFDataValidation) {
            if (fromValue >= 0) {
                dataValidation.createErrorBox("Thông báo lỗi", "Định dạng dữ liệu phải là số thập phân không âm!");
                dataValidation.setShowErrorBox(true);
            } else {
    ...
    public void createDropListColumn(XSSFWorkbook wb, XSSFSheet sheet, List<Map<String, Object>> categories,
                                     List<String> keyList, int row, int addRowNum, int col, XSSFSheet desSheet, String pass, String codeKey, String categoryCode, XSSFSheet tempSheet) {
        setDataToCategorySheet(sheet, categories, keyList, codeKey, categoryCode);
        StringBuilder formula = new StringBuilder("'").append(sheet.getSheetName()).append("'!$B$1:$B$").append(categories.size());
    ...
    public void setDataToCategorySheet(XSSFSheet sheet, List<Map<String, Object>> categories, List<String> keyList, String codeKey, String categoryCode) {
    ...
    public void createDropListColumnWithRefColumn(XSSFWorkbook wb, XSSFSheet crCategorySheet, List<Map<String, Object>> crCategories, List<Map<String, Object>> rfCategories, List<String> keyList, XSSFSheet desSheet, XSSFSheet tempSheet, int crCategoryRow, int addRowNum, int crCategoryCol, int rfCategoryCol, String pass, String compareColofCRCategory, String compareColofRFCategory, String codeKeyofCRCategory, boolean blankCell, String crCategoryCode, String rfCategoryCode) {
        ...
        if (!rfCategories.isEmpty() && !crCategories.isEmpty()) {
            rfCategories.forEach(f -> {
                List<Map<String, Object>> dm = crCategories.stream()
                        .filter(k -> k.get(compareColofCRCategory).equals(f.get(compareColofRFCategory))).collect(Collectors.toList());
                if (dm.size() > 0) {
                    ...
                    StringBuilder formula = new StringBuilder("'").append(crCategorySheet.getSheetName()).append("'!$B$").append(initRowStart).append(":$B$").append(startRow.get());
                    ...
                    StringBuilder formula2 = new StringBuilder("'").append(crCategorySheet.getSheetName()).append("'!$B$").append(initRowStart).append(":$").append(CellReference.convertNumToColString(keyList.size())).append("$").append(startRow.get());
                    nameTmp2.setRefersToFormula(formula2.toString());
                }
            });
        }
        int coldiff = rfCategoryCol - crCategoryCol;
        StringBuilder validation = new StringBuilder("INDIRECT(\"")
                .append(crSheetName)
        ...
        setXSSFListValidation(desSheet, validation.toString(), crCategoryRow, crCategoryRow + addRowNum, crCategoryCol, crCategoryCol);
    ...
    public void createDropListColumnWithRefColumnHoNuoi(XSSFWorkbook wb, XSSFSheet crCategorySheet, List<Map<String, Object>> crCategories, List<Map<String, Object>> rfCategories, List<String> keyList, XSSFSheet desSheet, XSSFSheet tempSheet, int crCategoryRow, int addRowNum, int crCategoryCol, int rfCategoryCol, String pass, String compareColofCRCategory, String compareColofRFCategory, String codeKeyofCRCategory, boolean blankCell, String crCategoryCode, String rfCategoryCode) {
        AtomicInteger startRow = new AtomicInteger(0);
        String crSheetName = crCategoryCode + "_" + crCategoryCol + "_" + rfCategoryCol + "_";
        String rfSheetName = "RF_" + crSheetName + "_";
            ...
            rfCategories.forEach(f -> {
                List<Map<String, Object>> dm = crCategories.stream()
                        .filter(k -> {
                            ...
                            return crValue != null && rfValue != null && crValue.toString().equals(rfValue.toString());
                        }).collect(Collectors.toList());
                ...
                if (!dm.isEmpty()) {
                    ...
                    String nameSuffix = (f.get(compareColofRFCategory) != null) ? f.get(compareColofRFCategory).toString() : "";
        ...
        for (int i = crCategoryRow; i <= (crCategoryRow + addRowNum); i++) {
            Row formulaRow = getRowWithCreate(tempSheet, i);
            String parentCellAddress = "'" + desSheet.getSheetName() + "'!" + CellReference.convertNumToColString(rfCategoryCol) + (i + 1);

            String vlookupToGetIdFormula = "VLOOKUP(" + parentCellAddress + ",'" + lookupSheetName + "'!$A:$B,2,FALSE)";
            String helperFormula = "IFERROR(\"" + crSheetName + rfCategoryCode + "\" & " + vlookupToGetIdFormula + ", \"\")";
            Cell helperCell = getCellWithCreate(formulaRow, helperColIndex);
            helperCell.setCellFormula(helperFormula);

            String validationFormula = "INDIRECT(" + tempSheet.getSheetName() + "!" + helperColName + (i + 1) + ")";
            setXSSFListValidation(desSheet, validationFormula, i, i, crCategoryCol, crCategoryCol);

            String currentDropdownCellAddress = "'" + desSheet.getSheetName() + "'!" + CellReference.convertNumToColString(crCategoryCol) + (i + 1);
            String vlookupTableRangeNameFormula = "IFERROR(\"" + rfSheetName + rfCategoryCode + "\" & " + vlookupToGetIdFormula + ", \"\")";
    ...
    public void createDropListColumnWithRefSheet(XSSFWorkbook wb, XSSFSheet crCategorySheet, XSSFSheet rfCategorySheet, List<Map<String, Object>> crCategories, List<Map<String, Object>> rfCategories, List<String> keyList, XSSFSheet desSheet, XSSFSheet tempSheet, int crCategoryRow, int rfCategoryRow, int addRowNum, int crCategoryCol, int rfCategoryCol, String pass, String compareColofCRCategory, String compareColofRFCategory, String codeKeyofCRCategory, boolean blankCell, String crCategoryCode, String rfCategoryCode) {
        String crSheetName = crCategoryCode + "_" + crCategoryCol + "_" + rfCategoryCol + "_";
        String rfSheetName = "RF_" + crSheetName + "_";
        XSSFSheet rfSheet = wb.getSheet(rfSheetName);
        if (rfSheet == null) {
        ...
        StringBuilder validation = new StringBuilder("INDIRECT(\"")
                .append(crSheetName)
                .append("\"&SUBSTITUTE(OFFSET(INDIRECT(\"'");
        validation.append(rfCategorySheet.getSheetName())
                ...
                .append(",")
                .append(rfCategoryCol).append(")),0,0),\" \",\"_\"))");
        setXSSFListValidation(desSheet, validation.toString(), crCategoryRow, crCategoryRow + addRowNum, crCategoryCol, crCategoryCol);
        ...
        for (int i = crCategoryRow; i <= (crCategoryRow + addRowNum); i++) {
            ...
            f.append(desSheet.getSheetName())
                    ...
                    .append(rfCategoryRow)
                    .append(",")
                    .append(rfCategoryCol).append(")),0,0),\" \",\"_\"))),5,FALSE), \"\")");
            formulaCell.setCellFormula(f.toString());
            XSSFFormulaEvaluator formulaEvaluator = wb.getCreationHelper().createFormulaEvaluator();
            formulaEvaluator.evaluateFormulaCell(formulaCell);
        }
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(ExcelExportForm.class)) {
                    ...
                    if (List.class.isAssignableFrom(field.getType())) {
                        ...
                        List<?> list = (List<?>) FieldUtils.readField(field, data, true);
                        if (CollectionUtils.isNotEmpty(list)) {
                            this.setDataInListMultilayerMerge(list, startRow, sheet, hasSamepleRow, list.get(0).getClass());
                        }
                    } else {
                        String cellAddress = fa.cell();
                        String[] cellAddressArray = fa.cellArray();
    ...
    protected void setDataInListMultilayerMerge(List<?> listDataInSheet, int startRow, Sheet sheet, boolean hasSampleRow, Class<?> type) {
        if (CollectionUtils.isEmpty(listDataInSheet)) {
            return;
        }
        int rowSampleIndex = startRow;
     ...
     * @param type
     */
    protected void setDataMultilayerMerge(Map<String, ExcelData> excelDataMap, Sheet sheet, Class<?> type) {
        final Field[] fields = type.getDeclaredFields();
        for (Field field : fields) {
    ...
    public Map<String, Object> validateRowInPattern(Row row, FormulaEvaluator formulaEvaluator, Map<String, Map<Integer, String>> objCheck, List<Integer> requires, List<Integer> indices) {
        ...
        patterns.put("fullDate", Pattern.compile("(0[1-9]|[12][0-9]|3[01]|[1-9])/(0[1-9]|1[012]|[1-9])/(1[0-9]{3}|[2-9][0-9]{3})"));
        ...
        patterns.put("phone", Pattern.compile("(0|84)(\\s|\\.)?((2[0-9][0-9])|(3[2-9])|(5[689])|(7[06-9])|(8[01-689])|(9[0-46-9]))(\\d)(\\s|\\.)?(\\d{3})(\\s|\\.)?(\\d{3})"));
        patterns.put("ref", Pattern.compile("[1-9]+[0-9]*-.+"));
        patterns.put("latitude", Pattern.compile("^[-+]?([1-8]?[0-9](\\.\\d+)?|90(\\.0+)?)$"));
        patterns.put("longitude", Pattern.compile("^[-+]?((1[0-7][0-9]|[1-9]?[0-9])(\\.\\d+)?|180(\\.0+)?)$"));
        ...
        try {
            ...
            for (int index : indices) {
                ...
                String cellValue = Optional.ofNullable(getNormalizedCellValue(cell, formulaEvaluator)).orElse("").trim();
                ...
                if (requires.contains(index) && cellValue.isEmpty()) {
                    ...
                    for (Map.Entry<String, Pattern> entry : patterns.entrySet()) {
                        ...
                        Map<Integer, String> validationMap = objCheck.getOrDefault(type, Collections.emptyMap());
    ...
    public String getNormalizedCellValue(Cell cell, FormulaEvaluator evaluator) {
        ...
        if (type == CellType.FORMULA) {
            ...
            switch (evaluated.getCellType()) {
                ...
                case STRING:
                    return StringUtils.isBlank(evaluated.getStringValue()) ? null : evaluated.getStringValue();
                case BOOLEAN:
    ...
    public Map<String, Object> validateRowInPattern(Row row, FormulaEvaluator formulaEvaluator, Map<String, Map<Integer, String>> objCheck, List<Integer> requires, List<Integer> indices, Map<Integer, Integer> lengthValidator) {
        Map<String, Object> result = new HashMap<>();
            ...
            for (int index : indices) {
                ...
                if (requires.contains(index) && cellValue.isEmpty()) {
                    ...
                    if (!lengthValidator.isEmpty()) {
                        Map<Integer, String> validationMap = objCheck.getOrDefault("length",
    ...
    public void createCategorySheetWithReferenceVerOther(XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet,
                                                         List<Map<String, Object>> categories,
                                                         List<Map<String, Object>> refCategories, Map<String, Object> params) {
        @SuppressWarnings("unchecked")
        List<String> keyList = (List<String>) params.get("keyList");
        ...
        if (isGenerateData) {
            ...
            for (Map<String, Object> ref : refCategories) {
                List<Map<String, Object>> _categories = categories.stream()
                        .filter(f -> f.get("parentCode").equals(ref.get("id") != null ? ref.get("id") : ref.get("code"))).collect(Collectors.toList());
        ...
        if (StringUtils.isNotBlank(refSheetNameParent)) {
            String LastColumnAddress = CellReference.convertNumToColString(wb.getSheet(refSheetNameParent).getRow(0).getLastCellNum() - 1);
            refFormulas = "INDEX(" + refSheetNameParent + "!$B$1:$B$" + lastRow;
            refFormulas = refFormulas + ",MATCH(INDIRECT(ADDRESS(ROW(),COLUMN()-1))," + refSheetNameParent + "!$" + LastColumnAddress + "$1:$" + LastColumnAddress + "$" + lastRow + ",0))";
        } else {
            refFormulas = "INDEX(" + refSheetName + "!$A$1:$A$" + refCategories.size();
            if (params.get("multiColumnMap") != null && params.get("multiColumnMap").toString() != null) {
                refFormulas = refFormulas + ",MATCH(CONCATENATE(INDIRECT(ADDRESS(ROW(),COLUMN()-2)),\"_\",INDIRECT(ADDRESS(ROW(),COLUMN()-1)))," + refSheetName + "!$D$1:$D$" + refCategories.size() + ",0))";
            } else
                refFormulas = refFormulas + ",MATCH(INDIRECT(ADDRESS(ROW(),COLUMN()-1))," + refSheetName + "!$B$1:$B$" + refCategories.size() + ",0))";
        }
    ...
    public void createCategorySheetInputOneInTwo(XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet,
                                                 List<Map<String, Object>> categories, Map<String, Object> params) {
        @SuppressWarnings("unchecked")
        List<String> keyList = (List<String>) params.get("keyList");
        String pass = (String) params.get("pass");
        ...
        for (int row = firstRow; row <= lastRow; row++) {
            String formula = "IF($" + columnName + (row + 1) + "<>\"\"," + srcSheet.getSheetName() + "!$B$" + (categories.size() + 10) + ":$B$" + (categories.size() + 50) + "," + srcSheet.getSheetName() + "!$B$1:$B$" + categories.size() + ")";
            DataValidationConstraint constraint = helper.createFormulaListConstraint(formula);
            DataValidation dataValidation = helper.createValidation(constraint, new CellRangeAddressList(row, row, firstCol, lastCol));
            if (dataValidation instanceof XSSFDataValidation) {
                dataValidation.setSuppressDropDownArrow(true);
                dataValidation.setShowErrorBox(true);
            } else {
    ...
    public <T> File setSingleDataToDynamicTemplate(T data, List<Integer> columnsToHide, String dynamicTitle, String template) {
        Class<?> classType = data.getClass();
        ExcelExportGeneralConfig config = classType.getAnnotation(ExcelExportGeneralConfig.class);
        if (config == null) {
            throw new CustomException("Lỗi config");
        }
        String templateName = template != null ? CommonConstant.EXCEL_TEMPLATE_FOLDER + template : this.getTemplateName(classType);
        File file = null;
        File templateFile = this.getTemplateFile(templateName);
        try (Workbook workbook = this.openWorkbook(templateFile)) {
            Sheet sheet = workbook.getSheetAt(0);
    ...
    public void createDropListColumnWithParam(XSSFWorkbook wb, XSSFSheet sheet, List<Map<String, Object>> categories,
                                              List<String> keyList, int row, int addRowNum, int col, XSSFSheet desSheet, String pass, String codeKey, String categoryCode, XSSFSheet tempSheet, Map<String, Object> params) {
        setDataToCategorySheet(sheet, categories, keyList, codeKey, categoryCode);
        int firstRow = (int) params.get("firstRow");
        int lastRow = (int) params.get("lastRow");
        ...
        if (colCheck != null && colCheck >= 0) {
            ...
            for (int r = firstRow; r <= lastRow; r++) {
                String formula = "IF($" + columnName + (r + 1) + "<>\"\",'" + sheet.getSheetName() + "'!$B$" + (categories.size() + 10) + ":$B$" + (categories.size() + 50) + ",'" + sheet.getSheetName() + "'!$B$1:$B$" + categories.size() + ")";
    ...
    public <T> File setSingleDataToTemplateMultiSheet(T data, List<Integer> columnsToHide, String dynamicTitle, List<Integer> removeCols) {
        Class<?> classType = data.getClass();
        ExcelExportGeneralConfig config = classType.getAnnotation(ExcelExportGeneralConfig.class);
        if (config == null) {
            throw new CustomException("Lỗi config");
    ...
    public void createDropListColumnAndReferToUnit(XSSFWorkbook wb, XSSFSheet sheet, List<Map<String, Object>> categories,
                                                   List<String> keyList, int row, int addRowNum, int col, XSSFSheet desSheet, String pass, String codeKey, String categoryCode, XSSFSheet tempSheet) {
        setDataToCategorySheet(sheet, categories, keyList, codeKey, categoryCode);
        StringBuilder formula = new StringBuilder("'").append(sheet.getSheetName()).append("'!$B$1:$B$").append(categories.size());
        setXSSFListValidation(desSheet, formula.toString(), row, row + addRowNum, col, col);
    ...
    private String generateNormalImportFormula(Map<String, Object> param) {
        ...
        String dataRage = mapGet(param, "dataRage", String.format("$B$%s:$Z%s",
                firstRow + 1, firstRow + 1 + dataLength));

        return String.format(
                "IFERROR(IF(INDEX('%s'!%s,ROW()+%d,COLUMN()+%d)=0,\"\"," +
                        "INDEX('%s'!%s,ROW()+%d,COLUMN()+%d)),\"\")",
                typingSheetName, dataRage, rowDistance, colDistance,
                typingSheetName, dataRage, rowDistance, colDistance
        );
    }
    ...
    public <T> File setSingleDataToTemplateGrouped(
            T data, String dynamicTitle, String dynamicSheetName, List<Integer> columnsToRemove,
            GroupSpec spec
    ) {
        Class<?> classType = data.getClass();
        ExcelExportGeneralConfig config = classType.getAnnotation(ExcelExportGeneralConfig.class);
        ...
        try (Workbook wb = openWorkbook(templateFile)) {
            ...
            if (spec != null && spec.isEnabled()) {
                ...
                int toCol = spec.getToCol() != null ? spec.getToCol() : detectLastDataCol(sheet, spec.getFromRow());
    ...
    private <T> void injectGroupHeaderRowsIntoDataList(T data, GroupSpec spec)
        ...
        Field listField = Arrays.stream(data.getClass().getDeclaredFields())
                .filter(f -> f.isAnnotationPresent(ExcelExportForm.class)
