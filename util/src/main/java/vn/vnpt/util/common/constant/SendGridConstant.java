...
public class SendGridConstant {
  ...
  public enum DeliveryEvent {
    PROCESSED(
        "processed", SendStatus.PENDING, "Message has been received and is ready to be delivered."),
    DROPPED(
        "dropped",
    ...
    DEFERRED(
        "deferred", SendStatus.SEND_FAILED, "Receiving server temporarily rejected the message."),
    BOUNCE(
        "bounce",
        SendStatus.SEND_FAILED,
    ...
    public static DeliveryEvent get(String value) {
      if (StringUtils.isBlank(value)) return DeliveryEvent.UNKNOWN;
  ...
  public enum EngagementEvent {
    ...
    public static EngagementEvent get(String value) {
      if (StringUtils.isBlank(value)) return EngagementEvent.UNKNOWN;
