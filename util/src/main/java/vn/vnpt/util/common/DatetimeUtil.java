...
public class DatetimeUtil {
  ...
  public static String formatDateToString(Date date, String pattern) {
    if (null == date) return "";
    if (StringUtils.isBlank(pattern)) pattern = DATE_FORMAT_3;
  ...
  public static String getCurrentDatetimeInFormat(String format) {
    if (StringUtils.isBlank(format)) format = DATE_FORMAT_3;
  ...
  public static LocalDate compareDateGetBefore(
      Optional<LocalDate> date1, Optional<LocalDate> date2) {
    return date1.isPresent() && date2.isPresent() && date1.get().compareTo(date2.get()) >= 0
        ? date2.get()
        : !date1.isPresent() && date2.isPresent() ? date2.get() : null;
  }

  public static LocalDate compareDateGetAfter(
      Optional<LocalDate> date1, Optional<LocalDate> date2) {
    return date1.isPresent() && date2.isPresent() && date1.get().compareTo(date2.get()) >= 0
        ? date1.get()
        : !date1.isPresent() && date2.isPresent() ? date2.get() : null;
  ...
  public static String formatLocalDateTimeToPattern(
      LocalDateTime dateTime, String pattern, ZoneId zoneId) {
  ...
  public static String formatTimestamp(Object timestamp) {
    ...
    try {
      ...
      if (timestampStr.contains("T")) {
        ...
        dateTime =
            LocalDateTime.parse(timestampStr, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.S"));
      } else {
        dateTime =
