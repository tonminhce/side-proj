...
public class CommonUtil {
   ...
   *
   * <p>This method processes the given lists of entities and DTOs to identify duplicates and
   * determine their status. It returns a result containing two maps:
   *
   * <p>- `countDup`: a map that counts the occurrences of each key extracted from the entities and
   * DTOs. - count equal 1 => exists in entities but not in DTOs, indicating it should be deleted. -
   * count equal 0 => exists in both entities and DTOs, indicating it should be updated. - count
   * equal -1 => exists in DTOs but not in entities, indicating it should be added to the database.
   ...
   * @param keyExtractDupEntity function to extract the key from an entity for identifying
   *     duplicates
   ...
   */
  public static <T, U, V> HandleDuplicateResult<T, U, V> handleDuplicate(
      List<T> entities,
      List<U> dtos,
  ...
  public static <T, U> HandleDuplicateResult<T, U, U> handleDuplicate(
      List<T> entities, List<U> dtos, Function<T, U> keyExtractDupEntity) {
  ...
  public static <T, U> List<U> modelMapperList(List<T> dataList, Class<U> aClass) {
    ...
    return dataList.stream().map(data -> modelMapper(data, aClass)).collect(Collectors.toList());
  }
