 ...
 *
 * <p>Class này chỉ xử lý phần kỹ thuật lặp lại như validate file, đọc DTO từ Excel, đổi file
 * template sang byte[], tạo header download và merge cell. Không đặt validate nghiệp vụ, map danh
 * mục, lưu DB hoặc audit ở đây.
 */
public final class ExcelImportExportHelper {
   ...
   *
   * <p>HDSD: gọi ở đầu hàm pre-import, trước khi tạo {@link XSSFWorkbook}. Hàm kiểm tra 3 lỗi kỹ
   * thuật thường gặp: file rỗng, vượt dung lượng cho phép, và tên file không kết thúc bằng {@code
   * .xls} hoặc {@code .xlsx}.
   *
   * <p>Code mẫu:
   *
   ...
   *
   * <p>HDSD: dùng sau {@link #validateExcelFile(MultipartFile, long, String, String, String)}. DTO
   * truyền vào cần khai báo các annotation mà {@link ExcelReader} đang hỗ trợ như
   * {@code @ExcelImportConfig}, {@code @ExcelImport}, {@code @ExcelImportLineIndex}. Hàm tự mở/đóng
   * workbook và trả về {@code rowDataList}.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static <T> List<T> readRows(
      MultipartFile file, Class<T> dtoClass, ExcelReader excelReader) {
    if (excelReader == null) {
   ...
   *
   * <p>HDSD: sau khi đọc rows, truyền vào hàm lấy mã dòng như {@code Dto::getCode} và chuỗi nhận
   * diện dòng ví dụ, ví dụ {@code "Dòng ví dụ minh họa"}. Hàm giữ lại các dòng có mã khác null và
   * mã không chứa chuỗi ví dụ.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   */
  public static <T> List<T> filterExampleRows(
      List<T> rows, Function<T, String> codeGetter, String exampleText) {
   ...
   *
   * <p>HDSD: gọi sau khi lọc dòng ví dụ và trước khi validate nghiệp vụ từng dòng. Hàm chỉ so sánh
   * số lượng, không kiểm tra nội dung từng dòng.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng cho case export/template chỉ cần truyền DTO và không cần đổi tên sheet, đổi tiêu
   * đề, xóa cột hoặc ẩn cột. Đây là wrapper gom điểm gọi {@code
   * TemplateExcelWriter.setSingleDataToTemplate(data)} về một nơi dễ tìm.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng thay cho {@code TemplateExcelWriter.setSingleDataToTemplate(data,
   * dynamicSheetName, dynamicTitle, removeColumns)} khi không cần ẩn cột. Nếu không cần tham số
   * nào, truyền {@code null} như cách code hiện tại đang dùng.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  /**
   * Ghi dữ liệu vào template Excel một sheet, có hỗ trợ đổi tên sheet, đổi tiêu đề, xóa cột và ẩn
   * cột.
   *
   * <p>HDSD: dùng thay cho {@code TemplateExcelWriter.setSingleDataToTemplate(data,
   * dynamicSheetName, dynamicTitle, columnsToRemove, columnsToHide)}. Đây là overload đầy đủ nhất
   * của nhóm single template.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static <T> File writeSingleTemplate(
    ...
    requireExcelWriter(excelWriter);
    return excelWriter.setSingleDataToTemplate(
        data, dynamicSheetName, dynamicTitle, columnsToRemove, columnsToHide);
  }

  /**
   * Ghi dữ liệu vào template Excel nhiều sheet bằng {@link TemplateExcelWriter}.
   *
   * <p>HDSD: dùng thay cho {@code TemplateExcelWriter.setSingleDataToTemplateMultiSheet(data,
   * columnsToHide, dynamicTitle, removeCols)} ở các case template có nhiều sheet hoặc DTO có nhiều
   * list gắn {@code @ExcelExportForm}.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static <T> File writeMultiSheetTemplate(
      ...
      List<Integer> removeColumns) {
    requireExcelWriter(excelWriter);
    return excelWriter.setSingleDataToTemplateMultiSheet(
        data, columnsToHide, dynamicTitle, removeColumns);
  }

  /**
   * Ghi dữ liệu vào các sheet cố định của template bằng {@link TemplateExcelWriter}.
   *
   * <p>HDSD: dùng thay cho {@code TemplateExcelWriter.setSingleDataToTemplateSheet(data)} ở các
   * case DTO có nhiều field list và mỗi field chỉ định sheet index bằng annotation. Khác với {@link
   * #writeMultiSheetTemplate(TemplateExcelWriter, Object, List, String, List)}, hàm này không nhận
   * cấu hình ẩn/xóa cột hoặc title động.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng sau các hàm {@code TemplateExcelWriter.setSingleDataToTemplate...} đang trả về
   * {@link File}. Nếu xóa ngay không thành công, file sẽ được đánh dấu {@code deleteOnExit()} để
   * JVM dọn sau.
   *
   * <p>Code mẫu:
   *
   ...
   *
   * <p>HDSD: dùng trong controller khi trả {@code ResponseEntity<byte[]>}. Ví dụ: {@code
   * ResponseEntity.ok().headers(downloadHeaders("Import.xlsx")).body(bytes)}. Hàm mặc định
   * content-type theo XLSX vì đa số template hiện tại dùng {@code XSSFWorkbook}.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng khi cần trả {@code .xls} hoặc loại Excel khác. Nếu {@code type} không có media
   * type tương ứng trong {@link ExcelType}, hàm vẫn tạo {@code Content-Disposition} để browser tải
   * file đúng tên.
   *
   * <p>Code mẫu:
   *
   ...
   *
   * <p>HDSD: gọi khi cần gộp nhiều cột trong cùng một row, ví dụ tạo tiêu đề bảng: {@code
   * mergeHorizontal(sheet, 0, 0, 5)} sẽ merge vùng A1:F1. Hàm trả về {@link CellRangeAddress} để
   * caller có thể set border/style bổ sung nếu cần.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   */
  public static CellRangeAddress mergeHorizontal(
      Sheet sheet, int rowIndex, int firstCol, int lastCol) {
   ...
   * {@code mergeVertical(sheet, 1, 4, 0)} sẽ merge vùng A2:A5. Hàm trả về {@link CellRangeAddress}
   * để caller có thể set border/style bổ sung nếu cần.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   * @return vùng cell đã merge.
   */
  public static CellRangeAddress mergeVertical(
      Sheet sheet, int firstRow, int lastRow, int colIndex) {
   ...
   *
   * <p>HDSD: dùng khi caller đã có đủ 4 tọa độ vùng cần merge. Index dòng/cột bắt đầu từ 0. Ví dụ
   * {@code mergeRegion(sheet, 0, 1, 0, 3)} sẽ merge vùng A1:D2. Nếu chỉ cần merge ngang hoặc dọc,
   * ưu tiên dùng {@link #mergeHorizontal(Sheet, int, int, int)} hoặc {@link #mergeVertical(Sheet,
   * int, int, int)} để code dễ đọc hơn.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   */
  public static CellRangeAddress mergeRegion(
   ...
   *
   * <p>HDSD: dùng cho tiêu đề hoặc nhãn nhóm khi vừa muốn merge vừa muốn set text/style. Hàm chỉ
   * ghi dữ liệu vào ô đầu vùng merge theo quy ước của Excel; các ô còn lại trong vùng merge không
   * cần set value. Nếu truyền {@code style}, style sẽ được gắn cho ô đầu.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng khi cần ghi một header đơn lẻ vào sheet. Index dòng/cột bắt đầu từ 0. Hàm tự tạo
   * row/cell nếu chưa tồn tại, ghi value vào cell và set style nếu có.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static Cell createHeader(
      Sheet sheet, int rowIndex, int columnIndex, Object value, CellStyle style) {
   ...
   * vì Excel chỉ hiển thị value của ô đầu vùng merge. Index dòng/cột bắt đầu từ 0.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   * @return cell header vừa tạo hoặc cập nhật.
   * @throws CustomException khi vị trí header không phải ô đầu vùng merge hoặc vùng merge không hợp
   *     lệ.
   */
  public static Cell createHeader(
      Sheet sheet,
   ...
   * Index dòng/cột bắt đầu từ 0. Hàm tự tạo row/cell nếu chưa tồn tại, ghi value theo kiểu dữ liệu
   * cơ bản và set style nếu có.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng khi cần ghi body row thủ công. Ví dụ ghi mã, tên, trạng thái theo thứ tự từ cột B
   * thì truyền {@code startColumnIndex = 1}. Hàm trả về row để caller có thể set height hoặc thao
   * tác bổ sung nếu cần.
   *
   * <p>Code mẫu:
   *
  ...
  public static Row createRowCells(
      Sheet sheet, int rowIndex, int startColumnIndex, List<?> values, CellStyle style) {
   ...
   *
   * <p>HDSD: gọi sau khi đã ghi xong dữ liệu vào sheet. Index cột bắt đầu từ 0. Hàm chạy {@code
   * sheet.autoSizeColumn(columnIndex)} cho từng cột trong khoảng {@code
   ...
   *
   * <p>HDSD: dùng sau khi merge hoặc sau khi tạo bảng để set border/alignment/background đồng nhất
   * cho cả vùng. Hàm tự tạo row/cell còn thiếu trong vùng để đảm bảo style được apply đủ các ô.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng khi caller đã có sẵn formula trỏ tới vùng dữ liệu danh mục, named range hoặc công
   * thức {@code INDIRECT}. Index dòng/cột bắt đầu từ 0. Đây là primitive chung cho các hàm tạo
   * dropdown danh mục bên dưới.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng để chuẩn bị sheet ẩn chứa danh mục trước khi gắn dropdown. Mỗi phần tử trong
   * {@code categories} là một dòng; mỗi key trong {@code keyList} là một cột. Thường quy ước cột A
   * là mã/id, cột B là tên hiển thị cho dropdown.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   *
   * <p>HDSD: dùng cho danh mục không phụ thuộc cột khác. Hàm ghi {@code categories} vào {@code
   * srcSheet}, lấy cột B của sheet đó làm label hiển thị, gắn dropdown vào {@code desSheet}, sau đó
   * ẩn và protect sheet danh mục nếu có mật khẩu.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static void createCategorySheet(
    ...
    setXssfListValidation(
        desSheet,
        srcSheet.getSheetName() + "!$B$1:$B$" + categories.size(),
        firstRow,
        lastRow,
        firstCol,
        lastCol);
  ...
  /**
   * Tạo sheet danh mục ẩn và gắn dropdown đơn giản bằng cấu hình map giống {@link
   * TemplateExcelWriter}.
   *
   * <p>HDSD: dùng khi muốn chuyển dần call site cũ từ {@code
   * TemplateExcelWriter.createCategorySheet} sang helper mà không đổi cấu trúc tham số. {@code
   * params} cần có {@code keyList}, {@code firstRow}, {@code lastRow}, {@code firstCol}, {@code
   * lastCol}; {@code pass} và {@code isGenerateData} là tùy chọn.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
   ...
   * vào sheet nguồn, tạo named range dạng {@code srcSheetName_parentId}, rồi gắn validation cho cột
   * con trên sheet đích. {@code params} tương thích với hàm cũ trong {@link TemplateExcelWriter}:
   * cần {@code keyList}, {@code refSheetName}, {@code firstRow}, {@code lastRow}, {@code firstCol},
   * {@code lastCol}; có thể truyền thêm {@code refParentCode}, {@code refCode}, {@code
   * distanceToRef}, {@code pass}, {@code isGenerateData}.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static void createCategorySheetWithReference(
      ...
      Map<String, Object> params) {
    createCategorySheetWithReferenceInternal(
        wb, srcSheet, desSheet, categories, refCategories, params, false);
  }

  /**
   * Tạo dropdown danh mục phụ thuộc danh mục cha, map theo code thay vì name.
   *
   * <p>HDSD: dùng cho case tên danh mục cha có thể trùng nhau nên không nên dò theo label. Hàm tạo
   * công thức phụ thuộc cột code kế bên theo logic cũ của {@code
   * TemplateExcelWriter.createCategorySheetWithReferenceCode}.
   *
   * <p>Code mẫu:
   *
   * <pre>{@code
  ...
  public static void createCategorySheetWithReferenceCode(
      ...
      Map<String, Object> params) {
    createCategorySheetWithReferenceInternal(
        wb, srcSheet, desSheet, categories, refCategories, params, true);
  }

  /**
   * Render dropdown ref chuẩn kiểu dynamic-import: sheet ref ẩn + named range + validation.
   *
   * <p>HDSD: dùng khi đã query được danh sách ref từ DB và muốn tạo dropdown độc lập với {@code
   * TemplateBuilderService}. Hàm tạo sheet ref có 2 cột {@code value}/{@code label}, ghi dữ liệu từ
   * {@code rows}, tạo named range trỏ tới cột label, ẩn sheet ref và gắn dropdown vào cột đích của
   * {@code targetSheet}.
   *
   * <p>Code mẫu:
   *
  ...
  public static void renderStandardRefDropdown(
    ...
    setXssfListValidation(
        targetSheet, namedRangeName, firstDataRow, lastDataRow, targetColIdx, targetColIdx);
  }
  ...
  private static void createCategorySheetWithReferenceInternal(
    ...
    if (getBooleanParam(params, "isGenerateData", true)) {
      ...
      for (Map<String, Object> ref : refCategories) {
        ...
        List<Map<String, Object>> childCategories =
            categories.stream()
                .filter(
                    category ->
                        Objects.equals(
        ...
        name.setNameName(
            srcSheetName
                ...
                + stringValue(
                    firstNonNull(
                        ref == null ? null : ref.get("id"), ref == null ? null : ref.get("code"))));
  ...
  private static void validateWorkbookAndSheets(
      XSSFWorkbook wb, XSSFSheet srcSheet, XSSFSheet desSheet) {
    if (wb == null) {
      throw new CustomException("Workbook không được null");
    }
    if (srcSheet == null) {
  ...
  private static boolean getBooleanParam(
      Map<String, Object> params, String key, boolean defaultValue) {
    Object value = params == null ? null : params.get(key);
  ...
  private static void validateCellRegion(
      Sheet sheet, int firstRow, int lastRow, int firstCol, int lastCol) {
    if (sheet == null) {
      throw new CustomException("Sheet không được null");
    }
  ...
  private static void validateMergeRegion(
      Sheet sheet, int firstRow, int lastRow, int firstCol, int lastCol) {
    if (sheet == null) {
      throw new CustomException("Sheet không được null");
    }
