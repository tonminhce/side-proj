...
public class StringUtil {
  ...
  public static String generatePassword() {
    ...
    password.append(
        specialChars.charAt(secureRandom.nextInt(specialChars.length()))); // Ký tự đặc biệt
  ...
  public static String concatAddress(
      String addressDetail, String communeName, String provinceName) {
  ...
  public static Long parseToLong(Object value) throws Exception {
    ...
    throw new CustomException(
        "Giá trị không hợp lệ, phải là Long hoặc String: " + value.getClass().getName());
  }
  ...
  public static String EscapeSpecialCharacterForSearch(String str) {
    ...
    Pattern SPECIAL_REGEX_CHARS =
        Pattern.compile("[%_'\"\\\\]"); // %, _, a single quote, a double quote, backslash
   ...
   *
   * <p>Nếu chuỗi input là null hoặc rỗng, trả về null. Nếu không, tự động thêm ký tự escape (\)
   * trước các ký tự đặc biệt: % và _
   *
   * @param input Chuỗi đầu vào cần xử lý
   * @return Mẫu LIKE sau khi escape và bọc '%' hai đầu, hoặc null nếu input null/rỗng
   *     <p>Ví dụ: - escapeLikeSearchPattern(null) -> null - escapeLikeSearchPattern("") -> null -
   *     escapeLikeSearchPattern("Nguyễn % Văn A") -> "%nguyễn \% văn a%" -
   *     escapeLikeSearchPattern("Lê _ Hải") -> "%lê \_ hải%" - escapeLikeSearchPattern("Trần Văn
   *     B") -> "%trần văn b%"
   */
