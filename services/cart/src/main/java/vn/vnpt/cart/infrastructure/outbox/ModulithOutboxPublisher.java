...
/**
 * Modulith outbox publisher — Story 2.1 (ADR-01 / ADR-04 / ADR-14 / ADR-20).
 *
 * <p>Mirrors {@code vn.vnpt.inventory.infrastructure.outbox.ModulithOutboxPublisher} with two
 * responsibilities: (1) persist the event to the {@code outbox} table in the same transaction as the
 * business state (ADR-04 atomicity); (2) publish via {@link ApplicationEventPublisher} so in-process
 * {@code @ApplicationModuleListener}s fire.
 *
 * <p>ponytail: cart's events are plain Lombok {@code @Value} POJOs (not Avro {@code SpecificRecord}),
 * so the inventory publisher's Avro-to-Map branch is dropped — Jackson serializes the event directly.
 * The {@code signatures} map (computed by {@code CartEventPublisher} over the JCS-canonical payload)
 * is persisted to the {@code outbox.signatures} JSONB column.
 */
@Component
public class ModulithOutboxPublisher implements OutboxPublisher {
  ...
  public void append(
    ...
    // Fire in-process listeners (none in Story 2.1; wired for Story 2.2's cart.line.added consumers).
