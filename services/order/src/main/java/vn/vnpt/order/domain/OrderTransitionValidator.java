...
/**
 * Order state transition validator — Story 4.1 + 4.2 + 4.4. The forward map covers the saga
 * state machine; Story 4.4 adds AMENDED and CANCELLED as valid transitions from any
 * non-terminal state (the 30-min edit window is enforced at the use-case layer, not here).
 */
@Component
public class OrderTransitionValidator {
  ...
  static {
    ...
    ALLOWED.put(OrderState.PAID, EnumSet.of(OrderState.ALLOCATED, OrderState.AMENDED, OrderState.CANCELLED));
